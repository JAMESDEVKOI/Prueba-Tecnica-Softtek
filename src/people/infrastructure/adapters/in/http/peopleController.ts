...
export class PeopleController
  implements Handler<APIGatewayProxyEventV2, Partial<Context>>
{
  ...
  async getPeopleList(_: APIGatewayProxyEventV2) {
    try {
      ...
      const results = [...peoples, ...response];
  ...
  async createPeople(
    ...
    try {
      const response = await this.peopleUseCase.addPeople(
        event.body as unknown as People
      );
  ...
  async updatePeople(
    ...
    const { id } = event.pathParameters!;
    try {
      const response = await this.peopleUseCase.updatePeople(
        id as string,
        event.body as unknown as People
      );
  ...
  async deletePeople(
    ...
    const { id } = event.pathParameters!;
    try {
      const response = await this.peopleUseCase.deletePeople(id as string);
